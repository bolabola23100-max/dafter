...
class PurchaseDocumentService {
  ...
  PurchaseDocumentService({ProductRepository? productRepository, SupplierRepository? supplierRepository})
  ...
  Future<Uint8List> buildPdf(Purchase purchase) async {
    ...
    final supplier = purchase.supplierId == null ? null : await _supplierRepository.getSupplierById(purchase.supplierId!);
          ...
          pw.Directionality(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                pw.Text('رقم الفاتورة: ${purchase.displayInvoiceNumber}', style: pw.TextStyle(font: font)),
  ...
  Future<bool> savePdf(Purchase purchase) async {
    final location = await getSaveLocation(suggestedName: 'فاتورة_شراء_${purchase.displayInvoiceNumber}.pdf', acceptedTypeGroups: const [XTypeGroup(label: 'PDF', extensions: ['pdf'])]);
    if (location == null) return false;
    final bytes = await buildPdf(purchase);
    await XFile.fromData(bytes, name: 'فاتورة_شراء_${purchase.displayInvoiceNumber}.pdf', mimeType: 'application/pdf').saveTo(location.path);
    return true;
  }
