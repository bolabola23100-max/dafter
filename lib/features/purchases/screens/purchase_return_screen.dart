...
class _PurchaseReturnScreenState extends State<PurchaseReturnScreen> {
  ...
  void initState() {
    ...
    if (widget.purchaseId != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _loadPurchase(widget.purchaseId!);
      });
    }
  }
  ...
  Future<void> _choosePurchase() async {
    try {
      ...
      if (purchases.isEmpty) {
        _message('مفيش فواتير شراء متاحة للمرتجع');
        return;
      }
            ...
            child: ListView.separated(
              ...
              itemBuilder: (_, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Text(
                    'التاريخ: ${_date(purchase.date)}  •  ${purchase.items.length} أصناف',
                  ),
                  trailing: Text(
      ...
      if (mounted) _message('تعذر تحميل فواتير الشراء: $e');
    }
  }
  ...
  Future<void> _loadPurchase(String id) async {
    ...
    try {
      ...
      if (mounted) _message('حصلت مشكلة وأنا بجيب الفاتورة: $e');
    } finally {
  ...
  void _setQuantity(ReturnItem item, int quantity) {
    ...
    setState(() {
      _items[index] = item.copyWith(
        quantity: quantity.clamp(0, item.maxQuantity),
      );
    });
  }
  ...
  Future<void> _save() async {
    if (_saving) return;
    if (_purchase == null) {
      _message('اختار فاتورة الشراء الأول');
      return;
    }

    final selected = _items.where((item) => item.quantity > 0).toList();
    if (selected.isEmpty) {
      _message('اختار صنف وكمية للمرتجع');
      return;
    }

    if (!_refundedAmount.isFinite ||
        _refundedAmount < 0 ||
        _refundedAmount > _total) {
      _message('راجع مبلغ الفلوس الراجعة');
      return;
    }

    if (_refundedAmount > 0 && _refundAccount == null) {
      _message('اختار الحساب اللي الفلوس هتنزل فيه');
      return;
    }
    ...
    try {
      ...
      final returnItems = selected.map((item) {
      ...
      await _returnService.createReturn(
        ...
        refundAccountId: _refundAccount?.id,
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              FilledButton.icon(
                                onPressed: _choosePurchase,
                                icon: const Icon(Icons.search),
                                label: const Text('اختيار فاتورة'),
                              ),
                            ],
                          ),
                          if (_purchase != null) ...[
                            ...
                            Text(
                              'إجمالي الفاتورة: ${_purchase!.total.toStringAsFixed(2)} ج.م',
                            ),
                          ],
                        ],
                      ),
                      ...
                      Column(
                        children: [
                          CustomTextFormField(
                            ...
                            onChanged: (value) {
                              setState(() {
                                _refundedAmount = double.tryParse(value) ?? 0;
                              });
                            },
                          ),
                            ...
                            DropdownButtonFormField<Account>(
                              ...
                              items: _accounts.map((account) {
                                return DropdownMenuItem<Account>(
                                  value: account,
                                  child: Text(
                                    '${account.name} - ${account.balance.toStringAsFixed(2)} ج.م',
                                  ),
                                );
                              }).toList(),
                              onChanged: (value) {
                                setState(() => _refundAccount = value);
                              },
                            ),
                          ],
                          const SizedBox(height: 12),
                    ...
                    FilledButton.icon(
                      ...
                      label: Text(_saving ? 'بيحفظ...' : 'حفظ المرتجع'),
