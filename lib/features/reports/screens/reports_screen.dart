...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Future<void> _openFilter() async {
    final result = await Navigator.push<Map<String, String>>(
      context,
      MaterialPageRoute(builder: (_) => ReportFilterScreen(initialPeriod: selectedPeriod, initialReport: selectedReport)),
    );
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              const Expanded(child: Text('ملخص التقارير', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold))),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 7),
                decoration: BoxDecoration(color: const Color(0xFFF1F3F4), borderRadius: BorderRadius.circular(8)),
                child: Text('$selectedPeriod • $selectedReport', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
              ),
            ],
          ),
