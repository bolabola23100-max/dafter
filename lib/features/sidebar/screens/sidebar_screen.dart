              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    ...
                    const Text('dafter', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    const Text('نظام إدارة المخزون', style: TextStyle(fontSize: 12, color: Colors.grey)),
                    const SizedBox(height: 20),
                    Expanded(
                      child: ListView(
                        padding: const EdgeInsets.symmetric(horizontal: 12),
                        children: [
                          NavItem(icon: Icons.home_outlined, label: 'الرئيسية', selected: _currentScreen == AppScreen.home, onTap: () => setState(() => _currentScreen = AppScreen.home)),
                          NavItem(icon: Icons.shopping_cart_outlined, label: 'المبيعات', selected: _currentScreen == AppScreen.sales, onTap: () => setState(() => _currentScreen = AppScreen.sales)),
                          NavItem(icon: Icons.inventory_2_outlined, label: 'المنتجات والمخزون', selected: _currentScreen == AppScreen.products, onTap: () => setState(() => _currentScreen = AppScreen.products)),
                          NavItem(icon: Icons.receipt_long_outlined, label: 'المشتريات', selected: _currentScreen == AppScreen.purchases, onTap: () => setState(() => _currentScreen = AppScreen.purchases)),
                          NavItem(icon: Icons.people_outline, label: 'العملاء', selected: _currentScreen == AppScreen.customers, onTap: () => setState(() => _currentScreen = AppScreen.customers)),
                          NavItem(icon: Icons.local_shipping_outlined, label: 'الموردين', selected: _currentScreen == AppScreen.suppliers, onTap: () => setState(() => _currentScreen = AppScreen.suppliers)),
