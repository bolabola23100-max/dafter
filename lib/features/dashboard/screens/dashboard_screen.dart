...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() {
        ...
        _saleReturns = results[1] as List;
  ...
  double get _todayProfit {
    ...
    for (final sale in _todaySales) {
      final cost = sale.items.fold<double>(0, (sum, item) => sum + item.costTotal);
      profit += sale.total - cost;
    }

    // A return reverses both the revenue and the historical cost of the sold
    // items. SaleItem.costPrice is the snapshot captured at sale time.
    for (final saleReturn in _saleReturns.where((r) => _sameDay(r.date, now))) {
      ...
      for (final item in saleReturn.items) {
        final sale = _sales.cast<Sale?>().firstWhere(
          (s) => s?.id == saleReturn.saleId,
          orElse: () => null,
        );
        final original = sale?.items.cast<dynamic>().firstWhere(
          (x) => x.saleId == saleReturn.saleId && x.id == item.saleItemId,
          orElse: () => null,
        );
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 20),
        SizedBox(height: 210, child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: data.map((item) {
