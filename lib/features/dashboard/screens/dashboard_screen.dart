...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() {
        ...
        _customerDue = (results[2] as List).fold<double>(
          0,
          (sum, item) => sum + (item.balance as double),
        );
        _supplierDue = (results[3] as List).fold<double>(
          0,
          (sum, item) => sum + (item.balance as double),
        );
        _loading = false;
  ...
  double get _todayTotal =>
      _todaySales.fold(0, (sum, sale) => sum + sale.total);

  double get _todayProfit {
    final prices = {
      for (final product in _products) product.id: product.purchasePrice,
    };
    return _todaySales.fold<double>(0, (sum, sale) {
      final cost = sale.items.fold<double>(
        0,
        (sum, item) => sum + costForItem(item, prices),
      );
