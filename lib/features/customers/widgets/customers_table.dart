...
class CustomersTable extends StatelessWidget {
  ...
  const CustomersTable({super.key, required this.customers, this.onViewCustomer, this.onRecordPayment, this.onDeleteCustomer});

  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFE5E9EB))), child: customers.isEmpty ? const Center(child: Padding(padding: EdgeInsets.all(40), child: Text('لسه مفيش عملاء'))) : SingleChildScrollView(child: DataTable(columnSpacing: 24, columns: const [DataColumn(label: Text('العميل')), DataColumn(label: Text('التليفون')), DataColumn(label: Text('الرصيد')), DataColumn(label: Text('إجراءات'))], rows: customers.map((c) => DataRow(cells: [
    ...
    DataCell(Row(children: [
      ...
      IconButton(icon: const Icon(Icons.delete_outline, size: 18, color: Colors.red), tooltip: 'حذف العميل', onPressed: () => onDeleteCustomer?.call(c)),
    ])),
  ])).toList()));
}
