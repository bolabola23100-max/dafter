...
class AppDatabase {
  ...
  Future<void> _createIndexes(Database db) async {
    await db.execute('''
      ...
      ON ${DatabaseTables.products}(barcode)
    ''');

    await db.execute('''
      CREATE INDEX idx_sale_items_sale_id
      ON ${DatabaseTables.saleItems}(sale_id)
    ''');

    await db.execute('''
      CREATE INDEX idx_sale_items_product_id
      ON ${DatabaseTables.saleItems}(product_id)
    ''');

    await db.execute('''
      CREATE INDEX idx_purchase_items_purchase_id
      ON ${DatabaseTables.purchaseItems}(purchase_id)
    ''');
    ...
    await db.execute('''
      ...
      ON ${DatabaseTables.stockMovements}(product_id)
    ''');

    await db.execute('''
      CREATE INDEX idx_account_transactions_account_id
      ON ${DatabaseTables.accountTransactions}(account_id)
    ''');
  }
}
